...
var availableFormIds = function(userId) {
  return _.pluck(
    FormBuilder.Collections.Forms.find(
      { createdUser: userId },
      { fields: { _id: 1 } }
    ).fetch(),
    '_id'
...
Meteor.publish('publicFormFields', function(formIds) {
  ...
  var publicForms = FormBuilder.Collections.Forms.find(
    { $and: [{ _id: { $in: formIds } }, { isPublic: true }] },
    { fields: { _id: 1 } }
  );
  var publicFormIds = _.pluck(publicForms.fetch(), '_id');

  return FormBuilder.Collections.Fields.find({
    formId: { $in: publicFormIds },
  });
});
...
Meteor.publish('submissionList', function(formId, limit, skip) {
  check(formId, String);
  check(limit, Number);
  check(skip, Number);
  ...
  Counts.publish(
    this,
    'submission_counter',
    FormBuilder.Collections.Submissions.find({
      $and: [
        { formId: { $in: availableFormIds(this.userId) } },
        { formId: formId },
      ],
  ...
  return FormBuilder.Collections.Submissions.find(
    {
      $and: [
        { formId: { $in: availableFormIds(this.userId) } },
        { formId: formId },
